/*
 * This program generates a PDF output file with a line shape (an angle) as an annotation to the
 * file. The program defines the vertices for the outlines of the annotation, and the line and
 * fill colors.
 *
...
{
    std::cout << "PolyLineAnnotation Sample:" << std::endl;

    Library lib;
    std::cout << "Initialized the library." << std::endl;

    std::string output_path = "PolyLineAnnotations-out.pdf";
    ...
    try {
        ...
        PolyLineAnnotation polyline_annot(page, rect, vertices, -1);
        std::cout << "Created new PolyLineAnnotation as 0th element of annotation array." << std::endl;

        // Retrieve and display the vertices
        auto vertices2 = polyline_annot.get_vertices();
        std::cout << "Retrieved the vertices of the polyline annotation." << std::endl;
        std::cout << "They are:" << std::endl;
        for (std::size_t i = 0; i < vertices2.size(); ++i)
            std::cout << "Vertex " << i << ": (" << vertices2[i].h << ", " << vertices2[i].v << ")" << std::endl;

        // Add line ending styles
        polyline_annot.set_start_ending_style(LineEndingStyle::ClosedArrow);
        polyline_annot.set_end_ending_style(LineEndingStyle::OpenArrow);

        // Set colors and generate an appearance stream
        polyline_annot.set_interior_color(Color{0.5, 0.3, 0.8});
        polyline_annot.set_color(Color{0.0, 0.8, 0.1});
        std::cout << "Set the stroke and fill colors." << std::endl;

        auto form = polyline_annot.generate_appearance();
        polyline_annot.set_normal_appearance(*form);
