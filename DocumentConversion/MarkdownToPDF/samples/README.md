...
From the sample directory, convert one file at a time:

```sh
./markdown_to_pdf samples/long.md long.pdf --overwrite --verbose
```

Use A4 landscape and a sans-serif font:

```sh
./markdown_to_pdf samples/long.md long.pdf --overwrite --verbose --page-size A4 --orientation landscape --font-family Helvetica --heading-font-family Helvetica --code-font-family Courier
```

Use installed system fonts and a CJK fallback font:

```sh
./markdown_to_pdf samples/multilingual-cjk-cyrillic.md multilingual.pdf --overwrite --verbose --cjk-font-family "PingFang SC"
```
